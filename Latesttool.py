...
#Handling '_type=BoolOp' Operations ex a=b>c
def boolop(BinaryTree):
    BinaryAssignment = ""
    rightOprand = ""
    try:
        if (BinaryTree["_type"] == "BoolOp"):
            boolBody = BinaryTree["values"]
            i=0
            for Jdata in boolBody:
               # if (Jdata["_type"] == "Compare" ):
                if (Jdata["comparators"][0]["_type"] == "Constant"):
                        if i != 0:
                            rightOprand += "and"
                        rightOprand += (str(Jdata["left"]["n"])if Jdata["left"]["_type"] == "Constant" else Jdata["left"]["id"])
                        rightOprand += mathOpps(Jdata["ops"][0]["_type"])
                        rightOprand += (str(Jdata["comparators"][0]["n"]) if Jdata["comparators"][0]["_type"] == "Constant"  else Jdata["comparators"][0]["id"])
                        #rightOprand += "and"
                
                if (Jdata["comparators"][0]["_type"] == "BinOp"):
                    rightOprand += (
                        str(Jdata["left"]["n"])
                        if Jdata["left"]["_type"] == "Constant"
                        else Jdata["left"]["id"]
                        )
                    opsBody = Jdata["ops"]
                    comparators = Jdata["comparators"]
                    i=-1
                    for Jdata in opsBody:
                        i += 1
                        if (comparators[i]["_type"] == "Constant" or comparators[i]["_type"] == "Name"):
        ...
        BinaryAssignment += rightOprand
    except:
        print('Error in boolop ')
    return BinaryAssignment
...
def SimpleAssignment(Jdata):
    ...
    try:
        ...
        if Jdata["value"]["_type"] == "Constant" or Jdata["value"]["_type"] == "Name":
            ...
            Assignments += LHS + boolop(Jdata["value"]) + "  "
        ...
        print('error in SimpleAssignment')
