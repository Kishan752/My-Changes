...
def Assignment(Jdata):
    global Assignmentss
    Assignmentss += SimpleAssignment(Jdata)
...
def handleIf(ifdata):
    global conditions
    global ifAssignments
    global handleFor
    if ifdata["_type"] == "If":
        ifBody = ifdata["body"]
        for Jdata in ifBody:
            if Jdata["_type"] == "Assign":
                ifAssignments += SimpleAssignment(Jdata)
...
def handleelse(ifdata):
    global conditions
    global elseAssignments
    global handleFor
    
    if ifdata["orelse"]:
        elBody = ifdata["orelse"]
        for Jdata in elBody:
            if ifdata["orelse"][0]["_type"] == "If":
                handleIf(ifdata["orelse"][0])
            elif ifdata["orelse"][0]["_type"] == "Assign":
                elseAssignments += SimpleAssignment(Jdata)
